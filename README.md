...
```
tanzu apps workload create steeltoe -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/steeltoe-source \
  --type web
```
...
```
tanzu apps workload get steeltoe
```
...
```
cf push steeltoe -p ./publish --random-route
```
